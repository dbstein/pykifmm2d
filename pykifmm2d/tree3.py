...
@numba.njit("(f8[:],f8[:],i8[:],b1[:],f8,f8[:],f8[:],i8[:],i8[:],b1[:],f8[:],f8[:],i8[:],i8[:],i8[:],i8[:],i8)", parallel=True)
def divide_and_reorder(x, y, ordv, tosplit, half_width, xmid, ymid, bot_ind, \
                top_ind, leaf, new_xmin, new_ymin, new_bot_ind, new_top_ind, parent_ind, children_ind, children_start_ind):
    """
...
def get_new_level(level, x, y, ordv, ppl):
    ...
    divide_and_reorder(x, y, ordv, to_split, level.half_width, level.xmid, level.ymid, \
            level.bot_ind, level.top_ind, level.leaf, xmin, ymin, bot_ind, top_ind, parent_ind, level.children_ind, 0)
    # construct new level
...
def split_bad_leaves(Level, Descendant_Level, x, y, ordv, bads):
    ...
    divide_and_reorder(x, y, ordv, bads, Level.half_width, Level.xmid, Level.ymid, \
            Level.bot_ind, Level.top_ind, Level.leaf, xmin, ymin, bot_ind, top_ind, parent_ind, Level.children_ind, Descendant_Level.n_node)
