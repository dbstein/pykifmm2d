...
def _on_the_fly_fmm(tree, tau, Nequiv, kernel_functions, numba_functions, verbose):
    ...
    E2C_Vs = []
    for ind in range(tree.levels):
        # instead of LU decomposition, we use the fact that these matrices
        # are circulant now
        # this saves Kernel Form time and is asymptotically faster
        # (whether its actually faster? depends)
        equiv_to_check = Kernel_Form(KF, small_xs[ind], small_ys[ind], \
                                        large_xs[ind][0:1], large_ys[ind][0:1])
        equiv_to_check_v = (1.0/np.fft.rfft(equiv_to_check[0]))[:,None]
        E2C_Vs.append(equiv_to_check_v)
    ...
    for ind in reversed(range(tree.levels)[1:]):
        ...
        Level.Equiv_Densities = fft_solve(E2C_Vs[ind], u_check_surfaces.T).T
        Level.RSEQD = np.reshape(Level.Equiv_Densities, Level.resh)
    et = time.time()
    ...
    for ind in range(1, tree.levels-1):
        ...
        local_expansions = fft_solve(E2C_Vs[ind], Level.Local_Solutions[doit].T).T
